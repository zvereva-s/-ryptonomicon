    ...
    subscribesToUpdates(tickerName){
      setInterval(async () => {
        const res = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=3688be9b4ea51e643243ddda7cb7cd2f30bc714610671215182e8aa3b502e442`
        )
      ...
      <template v-if="tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
            ...
            class="bg-white overflow-hidden shadow rounded-lg border-solid border-purple-800 cursor-pointer"
          >
            <div class="px-4 py-5 sm:p-6 text-center">
              <dt class="text-sm font-medium text-gray-500 truncate">{{ tk.name }} - USD</dt>
              <dd class="mt-1 text-3xl font-semibold text-gray-900">{{ tk.price }}</dd>
            </div>
            <div class="w-full border-t border-gray-200"></div>
            <button
              @click.stop="deleteTicker(tk)"
